...
def model_info(model, config):
    ...
    """
    if(config.model_type=='VARNET_MRI'):
        c = config
        input_size = (c.batch_size, 2 ,c.no_in_channel, c.height, c.width)       
        ...
        model_summary = summary(model, verbose=0, mode="train", depth=c.summary_depth,\
                                input_size=tuple((input_size,input_size,input_size)), col_names=col_names,\
                                row_settings=row_settings, dtypes=dtypes,\
                                device=config.device,sens_maps= torch.zeros(input_size,device=model.device),mask=torch.zeros(input_size,device=model.device) )
